...
class ImperativeActionTest extends AnyFunSuite with ScalaCheckDrivenPropertyChecks {
  ...
  test("onError is success") {
    val result = Try(onError("Hello", _ => throw new Exception("Boom")))
    assert(result == Success("Hello"))
  }

  test("onError is failure") {
    val result = Try(onError(throw new Exception("Boom"), _ => throw new Exception("Boom")))
    assert(result.isFailure)
  }
