...
object EitherExercises1 {
  ...
  sealed abstract class UserEmailError(message: String) {
    def getMessage: String = message
  }
  object UserEmailError {
    case class UserNotFound(userId: UserId)  extends UserEmailError(s"User ${userId.value} has no email address")
    case class EmailNotFound(userId: UserId) extends UserEmailError(s"User ${userId.value} has no email")
  }
  // In Scala 3,
  // enum UserEmailError {
